...
# TODO: colocar verificação de processamento da coleta
# USAR ESTE SCRIPT
# shf do arquivo slow do pefil, ok
# radiação da torre1 está no slow dos fluxos
#=====================================================================
...
#inpFile <-   '/home/emilia/lcb_2021/EXT/data/2021/21315/EP/TOA5_EP_21315.dat'
#inpFile <- '21196/EP2/TOA5_EP2_21196.dat'
cat(paste0('\n arquivo a processar, ', inpFile, '\n'))


#---- identificar arquivo de entrada ----
# Torre 1: EP ==> só tem dados de shf

if( substr(inpFile, nchar(inpFile)-12, nchar(inpFile)-9) == '_EP_' ){
  cat('Sistema Perfil 1\n') 
  # precisa acertar os caminhos desses arquivos
  
  new <- read.csv(inpFile, header=F, skip=4, na.strings = "NAN", stringsAsFactors = F)
  new <- data.frame(date = as.POSIXct(new$V1, tz='GMT'), new[,-1])
  names(new)[-1] <- c("volt_min_Min","PTemp_Avg","SHF1_Avg","SHF2_Avg","SHF3_Avg.")
  new$G <- apply(new[,c(4:6)],1,function(x) round( mean(x, na.rm=T),2))
  
  # arquivo antigo
  old <- read.csv("/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/extrema_shf_Torre1_15min.csv",
                  na.strings = c("NAN", "NA"), stringsAsFactors = F)
  old$date <- as.POSIXct(old$date, tz='GMT')
      # aux <- apply(old[,-1], 1, function(x)sum(is.na(x))) <<====== PRESTA ATENÇAO
  
  # merge
  out <- merge(old, new, all=T)
  ref <- data.frame(date=seq.POSIXt(min(out$date), max(out$date), by = '15 min'))
  if( dim(out)[1] < dim(ref)[1] ){
    out <- merge(ref, out, by="date", all.x=T)
  }
  #out <- old
  write.csv(out, "/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/extrema_shf_Torre1_15min.csv",
        row.names=F,quote = T)
  
  # 30min
  out30 <- timeAverage(out, avg.time = '30 min', start.date = paste0(substr(out$date[1],1,14),'00:00'))
  write.csv(out30, "/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/extrema_shf_Torre1_30min.csv",
            row.names=F,quote = T,)
  ...
  cat('Sistema Perfil 1\n')
  # ler arquivo novo,
  # ler arquivo velho, concatenar e sobreescrever
  # 
  # 
}else{
...
#.............................................................................................
# Lendo slow dos fluxos 

old <- read.csv('/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/EXT_Torre1_Radiation_10min.csv',
                na.strings = c("NAN", "NA",'-9999'), stringsAsFactors = F)
old$date <- as.POSIXct(old$date, tz='GMT')
#aux <- apply(old[,-1], 1, function(x)sum(is.na(x)))
# 
# #........................................................................................
# # novos arquivos
 
inp <- list.files('/home/emilia/lcb_2021/EXT/data/2021',
                  pattern = "TOA5_EF_", recursive = T, full.names = T)
inp <- inp[which(substr(inp, nchar(inp)-16, nchar(inp)-9) == "TOA5_EF_")]

dt <- lapply(inp, function(x)read.csv(x, skip = 4, header=F,
                                      na.strings = c("NAN", "NA",'-9999'),
                                      stringsAsFactors = F))
tt <- do.call(rbind, dt)
tt <- data.frame(date=as.POSIXct(tt$V1, tz='GMT', format="%Y-%m-%d %H:%M:%S"),
                 tt[,-1])
names(tt)[1:6] <- names(old)
out <- merge(old, tt[,1:6], all=T)

ref <- data.frame(date=seq.POSIXt(min(out$date), max(out$date), by = '10 min'))
...
#out <- old
write.csv(out,
          "/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/EXT_Torre1_Radiation_10min.csv",
          row.names=F,quote = T)

# 30min
out30 <- timeAverage(out, avg.time = '30 min', start.date = paste0(substr(out$date[1],1,14),'00:00'))
write.csv(out30,
          "/home/emilia/lcb_2021/EXT/outExt/Radiação_torre1/EXT_Torre1_Radiation_30min.csv",
          row.names=F,quote = T,)
