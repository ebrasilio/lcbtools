...
#year <- 2023
data_repo <- '/data1/DATA/LCB/EXT/data'
out <- '/data1/DATA/LCB/EXT/outExt'

for(year in 2014:2023){ #2020:2023){
  cat('*******  --> ', year, '\n')
  repo <- paste0(data_repo,'/',year)
  ...
  pdf(paste0(out, '/plt/origin_',year,".pdf"), width=12)
  #---- Process  ----
  for(i in 1:length(sta)){
  # if(year == 2017)  sta <- sta[-18]
    cat("\n\nProcessando estação: ", sta[i],'\n')
    new <- checkNewData(repo, sta[i]) # ta lendo tudo sem checagem
    dt <- new_dt(new, sta[i])
    if(!is.null(dt)){
...
for(i in 1:length(sta)){
  cat ('\n',' ', sta[i], '\n')  #'../outExt/Clima',sta[i]
  if( file.exists( paste0(out,'/Clima/', sta[i], 'clear_merge.txt')) ){
    t <- read.csv(paste0(out,'/Clima/', sta[i], 'clear_merge.txt') )
...
for(i in 1:length(sta)){
  f1 <- list.files(repo, pattern=paste0(sta[i],'clear_merge.txt'),  
  ...
  if(length(f1) != 0){
    ...
    printOuts(b, paste0(out,'/Clima'), sta[i])  
  } else {
    cat('\nEstação', sta[i],'sem dados \n')
  }
}
...
for(i in 1:length(sta)){
  aux <- read.csv(paste0(out, '/Clima/', sta[i], 'clear_merge.txt'), 
...
for( i in 1:length(stt) ){
  aux <- read.csv(paste0(out, '/Clima/', stt[i], 'clear_merge.txt'), 
                  na.strings=c(NA, '-9999'), stringsAsFactors=FALSE)
  aux$date <- as.POSIXct(aux$date, tz="GMT")
