...
# functions
exportData <- function(mydt=NULL, path, pattern){
  write.csv(mydt, paste0(path, 'EXT_Flx_', pattern, '_allvars.csv'), row.names = F, na = '-9999.0')
  write.csv(mydt[,which(names(mydt) %in% c("date","H","LE","co2_flux","u.","wind_speed","wind_dir"))],
            paste0(path, 'EXT_Flx_', pattern, '_vars.csv'), row.names = F, na = '-9999.0')
}
