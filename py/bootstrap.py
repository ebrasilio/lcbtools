...
class FillGap():
    ...
    def fillstation(self, stanames, all=None, plot = None, summary = None, From=None, To=None, by=None, 
                    how='mean', distance=None, sort_cor = True, constant =True, cor_lim=None, selected_variables=None):
        """
        DESCRIPTION
        ...
        INPUT
            ...
            constant: Use a constant in the linear regression
        OLD CODE:
                    try:
                        # get parameters
        ...
        for station in stations:
            ...
            if selected_variables == None:
                ...
                newdataframe = station.getData(var=selected_variables, reindex = True, From=From, To=To, by=by, how=how) # Dataframe which stock the new data of the stations
    ...
    #                 print newdataframe
            ...
            if 'U m/s' in newdataframe.columns and "V m/s" in newdataframe.columns:
...
def dist_matrix(self, stanames, lats, lons):
    """
    DESCRIPTION
...
#===============================================================================
    ...
    OutPath = '/data1/DATA/LCB/EXT/STORAGE/data/C/full/'
    Path_att = "/data1/DATA/LCB/EXT/STORAGE/doc/metadata/metadata_allnet.csv" # Path do arquivo dos attributos das estacoes que eu coloquei na sua maquina

    AttSta = att_sta() # create an instance of the station attributes object
    AttSta.setInPaths(inpath) # set the directory path of the data
    stanames= AttSta.stations(values=["Ribeirao"])  # From the metadata select the stations in the Ribeirao das Posses
    Files = AttSta.getatt(stanames, 'InPath') # get the name of the selected stations

    net = LCB_net() # Create an instance of the network object
    net.AddFilesSta(Files[:2]) # Add the stations to the networks
    stanames_in_net = net.getpara('stanames') # Get the stations effectively added to the network (Depend if their is data or not)

    From='2014-10-15 00:00:00'
    To='2017-08-01 00:00:00'

    distance = AttSta.dist_matrix(stanames_in_net) # Create the distance matrix of the station based on the latitude longitude
    gap = FillGap(net) # make an instance of the filling gap object
    gap.fillstation([], all = True, From=From, To=To, by='2min', how='mean',
                    summary=False, plot=False, distance=distance, sort_cor=True) # Fill the stations

    gap.WriteDataFrames(OutPath)


#==============================================================================
# Specific bootstraping for Rainfall
#==============================================================================
#     inpath ='/data1/DATA/LCB/EXT/STORAGE/data/C/merge/'
#     OutPath_rain = '/data1/DATA/LCB/EXT/STORAGE/data/C/full_rain/'
#     Path_att = "/data1/DATA/LCB/EXT/STORAGE/doc/metadata/metadata_allnet.csv" # Path do arquivo dos attributos das estacoes que eu coloquei na sua maquina
#
#     AttSta = att_sta(Path_att)  # create an instance of the station attributes object
#     AttSta.setInPaths(inpath)  # set the directory path of the data
#     stanames = AttSta.stations(values=["Head"])  # From the metadata select the stations in the Ribeirao das Posses
#     Files = AttSta.getatt(stanames, 'InPath')  # get the name of the selected stations
#
#     net = LCB_net()  # Create an instance of the network object
#     net.AddFilesSta(Files)  # Add the stations to the networks
#     stanames_in_net = net.getpara('stanames')  # Get the stations effectively added to the network (Depend if their is data or not)
#
#     From='2014-10-15 00:00:00'
#     To='2017-03-01 00:00:00'
#
#     distance = AttSta.dist_matrix(stanames_in_net)  # Create the distance matrix of the station based on the latitude longitude
#
#     gap = FillGap(net)
#     gap.fillstation([], all = True, From=From, To=To, by='H', how='sum', selected_variables = ['Rc mm'],
#                     summary=False, plot=False, distance=distance, constant=True, sort_cor=None, cor_lim=0.7)
#
#     gap.WriteDataFrames(OutPath_rain)
#
# # #------------------------------------------------------------------------------
# # #     # Perform another bootstrapping using the previous bootstraped data as input
#     net=LCB_net()
#     AttSta = att_sta(Path_att)
#     AttSta.setInPaths(OutPath_rain)
#     distance = AttSta.dist_matrix(stanames_in_net)
#     staPaths = AttSta.getatt(stanames_in_net , 'InPath')
